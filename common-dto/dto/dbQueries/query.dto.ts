export const queries =  {
     ...
     getWorkSchedule: 'SELECT schDay."day_of_week", schDay."id" as scheduleDayId,schIntr."id" as scheduleTimeId,  schIntr."start_time", schIntr."end_time" from doc_config_schedule_day schDay left  join doc_config_schedule_interval schIntr on schIntr."doc_config_schedule_day_id" = schDay."id" where schDay."doctor_id" = $1',
     getDoctorScheduleInterval: 'select dcday."doctor_id", dcday."day_of_week", dcday."doctor_key", dcint."start_time", dcint."end_time" from doc_config_schedule_day dcday join doc_config_schedule_interval dcint on dcday."id" = dcint."doc_config_schedule_day_id" where dcday."doctor_key" = $1 and dcday."day_of_week" = $2',
     ...
     getConfig: 'SELECT "consultation_session_timings","over_booking_type","over_booking_count","overbooking_enable" from doc_config where "doctor_key" = $1',
