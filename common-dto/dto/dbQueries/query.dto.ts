export const queries = {
    ...
    getDoctorByName: 'SELECT distinct d."doctorId", d."account_key" as "accountKey", d."doctor_key" as "doctorKey", d."speciality", d."photo",d."signature",d."number",d."first_name" as "firstName", d."last_name" as "lastName", d."registration_number" as "registrationNumber", config."consultation_cost" as fee , config."consultationSessionTimings" as sessionTiming, ac."street1" as street, ac."city" ,ac."state",ac."pincode", ac."country", ac."hospital_name" as "hospitalName", d."experience" as experience FROM doctor d left join doc_config config on config."doctor_key"=d."doctor_key" left join account_details ac on ac."account_key"=d."account_key" WHERE LOWER(doctor_name) LIKE LOWER($1) or LOWER(registration_number)  LIKE LOWER($1) or LOWER(d."doctor_key") LIKE LOWER($1) ',
    
    getHospitalByName: 'select "hospital_name" as "hospitalName", "phone" as "number","street1" as street, "city", "state","pincode","country", "account_key" as "accountKey" ,"hospital_photo" as photo from account_details WHERE hospital_name ~* $1',
    ...
    getPatientDoctorApps: 'SELECT * from appointment a join patient_details pd ON a."patient_id" = pd."patient_id" WHERE a."doctorId" = $1 AND  a."is_cancel"=false AND (pd.name ~* $2 OR pd.email ~* $2 OR pd.phone ~* $2)',
