export const queries = {
   ...
   // getAppointByDocId: 'select * from appointment where "doctorId" = $1 and appointment_date >= $2  order by appointment_date limit 7 ',
    ...
    getPastAppointments:'SELECT * FROM appointment WHERE "patient_id" = $1 AND "appointment_date" <= $2 AND "is_cancel"=false AND "status"= $3 order by (appointment."appointment_date",appointment."startTime") DESC',
    getUpcomingAppointments:'SELECT * FROM appointment WHERE "patient_id" = $1 AND "appointment_date" <= $2  AND "status"= $3 OR "status"=$4 AND "is_cancel"=false order by (appointment."appointment_date",appointment."startTime") DESC',
